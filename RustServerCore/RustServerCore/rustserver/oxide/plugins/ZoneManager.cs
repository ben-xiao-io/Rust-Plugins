...
{
    [Info("Zone Manager", "k1lly0u", "3.1.2")]
    ...
    public class ZoneManager : RustPlugin
    {
        ...
        private readonly Hash<NetworkableId, EntityZones> zonedEntities = new Hash<NetworkableId, EntityZones>();
        ...
        private object OnEntityTakeDamage(BaseCombatEntity entity, HitInfo hitinfo)
        {
            ...
            {
                if (attacker && HasEntityFlag(entity, ZoneFlags.NoBuildingDamage) && !CanBypass(attacker, ZoneFlags.NoBuildingDamage))
                    return true;
            }
            ...
            if (!(entity is LootContainer) && !(entity is BaseHelicopter))
            {
                ...
                {
                    if (attacker && CanBypass(attacker, ZoneFlags.UnDestr))
                        return null;
        ...
        private void OnPlayerSleepEnd(BasePlayer player)
        {
            if (!updateBehaviour)
            {
                Debug.LogError("[ZoneManager] UpdateBehaviour is null. Report this on uMod");
                return;
            }
        ...
        private static bool ReverseVelocity(BaseVehicle baseVehicle)
        {
            ...
            {
                if (baseVehicle.AnyMounted() && baseVehicle is BaseHelicopterVehicle)
                {
        ...
        private void ShowZone(BasePlayer player, string zoneId, float time = 30)
        {
            ...
            if (zone.definition.Size != Vector3.zero)
            {
                ...
                SendReply(player, $"ddraw.line, time, Color.blue, point7, point3");
            }
            else 
            { 
                player.SendConsoleCommand("ddraw.sphere", time, Color.blue, zone.definition.Location, zone.definition.Radius);
                Puts($"ddraw.sphere, {time}, {Color.blue}, {zone.definition.Location}, {zone.definition.Radius}");
            }
        ...
        private bool CanBypass(object player, int flag)
        {
            string flagName;

            if (ZoneFlags.IndexToName.TryGetValue(flag, out flagName))
                return permission.UserHasPermission(player is BasePlayer ? (player as BasePlayer).UserIDString : player.ToString(), PERMISSION_IGNORE_FLAG + flagName);
            else
            {
